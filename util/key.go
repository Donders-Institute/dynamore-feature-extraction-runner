...
// AddAuthorizedPublicKey appends the public key in `publicKeyFilename` into the
// file `.ssh/authorized_keys` within the home directory of `username`.
func AddAuthorizedPublicKey(username string, publicKeyFilename string) error {
	...
	// read public key from `publicKeyFilename`
	publicKeyBytes, err := ioutil.ReadFile(publicKeyFilename)
	if err != nil {
		return err
	}

	// write or append public key to ~/.ssh/authorized_keys
